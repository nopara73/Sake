...
public static class Decomposer
{
	...
	public static IEnumerable<(long Sum, int Count, ulong Decomposition)> Decompose(long target, long tolerance)
	{
		var denoms = StdDenoms.SkipWhile(x => x > target).ToArray();

		return denoms.SelectMany((_, i) => InternalCombinations(target, tolerance: tolerance / 10, maxLength: 8, denoms)).Take(100).ToList();
	}
